module ApplicationHelper
  ...
  def get_post_draft_key(post, type)
    if current_user and post.new_record?
      post.parent ? "draft_user_#{current_user.id.to_s}_reply_to_#{post.parent.id.to_s}_#{type}" : "draft_user_#{current_user.id.to_s}_new_post_#{type}"
    else
      ''
    end
  end
